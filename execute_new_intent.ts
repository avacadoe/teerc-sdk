...
const ERC20_ADDRESS = '0x2d13f85a3b201637Ad02F339b461749881f7d49d'; // TEST token

// Private key
const PRIVATE_KEY = process.env.PRIVATE_KEY || '95492791d9e40b7771b8b57117c399cc5e27d99d4959b7f9592925a398be7bdb';

// Intent execution parameters from transaction 0x103fa11f30a3e5d3243284fce8323ccbca6565c0a73a0700162b2a1543f90263
...
// Balance PCT (converted from decimal to hex)
const balancePCT = [
...
async function main() {
  console.log('🚀 Executing new withdrawal intent...\n');
  ...
  const tokenId = await publicClient.readContract({
    ...
    args: [ERC20_ADDRESS as `0x${string}`],
  });
  ...
  try {
    ...
    if (receipt.status === 'success') {
      ...
      console.log('\n✓ The TEST tokens have been sent to:', destination);
    } else {
      console.log('\n❌ Transaction failed!');
    ...
    } else if (error.message?.includes('UnknownToken')) {
      console.error('\n⚠️  TokenId not registered in contract.');
    }
